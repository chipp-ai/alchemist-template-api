...
#   - Their full codebase (standard Deno + Hono headless API template)
...
It is a standard Deno + Hono headless API that runs anywhere.
...
# 2. Start the dev stack (Postgres + Redis via Docker, then Deno API)
...
The app is now a standalone Deno + Hono headless API project.
