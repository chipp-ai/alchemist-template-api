...
# ===========================================================================
# dev.sh — Start the development stack (Deno API + Docker services)
#
# Usage:
#   ./scripts/dev.sh --api-port 8000 [--port PORT]
#
# --api-port is REQUIRED. --port is accepted for compatibility with callers
# that still pass it (e.g. desktop dev orchestrator for full-stack templates)
# but is ignored for this headless API template.
# ===========================================================================
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --api-port)
      ...
      ;;
    --port)
      # Accepted for compatibility with callers that still pass a Vite port
      # (e.g. desktop dev orchestrator for full-stack templates). Ignored
      # for this headless API template.
      shift 2
      ;;
    -h|--help)
      ...
      echo "  --port PORT        (optional, ignored for headless API template)"
      exit 0
      ;;
    *)
...
if [[ -z "$API_PORT" ]]; then
  echo "Error: --api-port is required."
  echo ""
  echo "Usage: ./scripts/dev.sh --api-port PORT [--port PORT]"
  echo "Example: ./scripts/dev.sh --api-port 8000"
  exit 1
fi
...
When docs reference \`__API_PORT__\`, use **$API_PORT**.
EOF
