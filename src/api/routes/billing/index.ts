 ...
 * The Stripe Checkout return page for purchases minted by the monetization
 * middleware (src/api/middleware/monetize.ts). Headless template -- there is
 * no SPA to land on, so this tiny server-rendered page closes the loop:
 * pay, come back, retry the API call.
 */
billingRoutes.get("/purchase/complete", (c) => {
  ...
  const body = canceled
    ...
    : "You can close this tab -- retry the API call and it will work now.";
 ...
 * spent by credit-priced routes (chargeCredits middleware).
 */
