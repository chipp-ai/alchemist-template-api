 ...
 *
 * This is a headless API template (no SPA). The dev-activity ring
 * buffers + GET/POST /api/dev/app-state still exist server-side so the
 * agent verification pipeline can read the running app's request/error
 * history; the client-snapshot half of the payload is simply empty when
 * no frontend is pushing to it.
 *
 * Coverage:
 *   - dev-activity ring buffer: append, cap, separate request/error
 *     buffers, snapshot returns a copy
 *   - recent-activity middleware registration gated on the dev-routes flag
 ...
 *     subsequent GETs
 *   - dev routes register POST and GET /app-state
 */
...
// Enable the fail-closed dev surface for the in-process dev-route tests.
// The router gate (devRoutesEnabled / ALCHEMIST_DEV_ROUTES) is evaluated
// per-request, so setting this before `.fetch()` is sufficient.
function withDevRoutes<T>(fn: () => Promise<T>): Promise<T> {
...
deno("source: app.ts mounts recentActivityMiddleware gated by the dev-routes flag", async () => {
  ...
  // Verify the registration is gated on the fail-closed dev-routes flag
  // (devRoutesEnabled / ALCHEMIST_DEV_ROUTES) — matching the gate on the
  // dev-routes router that EXPOSES this data, so the ring buffer never
  // accumulates customer-facing request metadata in production.
  if (!src.includes("devRoutesEnabled()")) {
    throw new Error(
      "app.ts must gate recentActivityMiddleware on devRoutesEnabled() so " +
        "the ring buffer stays off unless the dev surface is explicitly enabled.",
    );
  }
});
...
deno("e2e: GET /api/dev/app-state returns server context even with no client push", () =>
  withDevRoutes(async () => {
    __resetDevActivityForTests();

    // Late-import so the route module is evaluated with the dev flag on.
...
deno("e2e: POST /api/dev/app-state persists the client snapshot for subsequent GETs", () =>
  withDevRoutes(async () => {
    __resetDevActivityForTests();
...
deno("e2e: GET /api/dev/app-state?format=markdown returns text/markdown", () =>
  withDevRoutes(async () => {
    const { devRoutes } = await import("@/api/routes/dev/index.ts");
