...
// Note: This is the headless API template. There is no web/ SPA, so client-
// side source lints that inspected web/src/ are intentionally omitted.

deno("source: app.ts mounts recentActivityMiddleware gated on dev flag", async () => {
  ...
  // Verify the gate is on the dev-routes flag (ALCHEMIST_DEV_ROUTES).
  if (!src.includes("devRoutesEnabled()")) {
    throw new Error(
      "app.ts must gate recentActivityMiddleware on devRoutesEnabled(). " +
        "The ring buffer is harmless but accumulating customer-facing " +
        "request metadata when dev routes are disabled is unnecessary.",
    );
  }
});
...
deno("e2e: GET /api/dev/app-state returns server context even with no client push", async () => {
  ...
  // Late-import so the route module's guard captures the right env value.
