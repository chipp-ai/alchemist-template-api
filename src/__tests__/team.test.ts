 ...
 *   - Capability map exhaustiveness (every capability has a min-role entry)
...
deno("source: invite acceptance route is mounted publicly under /api/invite", async () => {
  // /invite/:token (preview) + /invite/:token/accept must resolve without
  // an org-scoped session — invitees aren't bound to an org yet. They live
  // in their own router mounted at /api/invite (NOT nested under /api/org).
  const src = await Deno.readTextFile(
    new URL("../api/routes/invite/index.ts", import.meta.url),
  );
  assertStringIncludes(
    src,
    "/:token",
    "invite routes must expose a token-parameterized preview/accept path",
  );
});
