...
Alchemist API Template is a headless API starter built on Deno 2 and Hono 4. It ships with auth, billing, RBAC, structured logging, and an idiomatic Kysely + services layout, plus a `CLAUDE.md` authored so AI agents (Claude Code, Cursor, or the [Alchemist AI](https://adaas.dev) platform itself) can navigate and extend it without fighting the conventions. Fork it as the starting point for any new API product.
...
This runs the Hono API on `:8000`. Use `curl` or any HTTP client to call the API endpoints.
...
The `/api/dev/*` routes 404 when `NODE_ENV=production` -- this surface is local-only by construction.
