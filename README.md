...
- **Billing** -- Stripe 17. Plan-tier subscriptions + customer portal, a product catalog (one-time + recurring, auto-created Stripe Products/Prices, webhook fulfillment, org entitlements), a prepaid credit ledger (atomic debits, credit-pack top-ups, per-cycle allowances), and THREE route-charging middlewares: requirePurchase (entitlement, 402 + checkout link), chargeCredits (metered, refund-on-failure), and mppPaid (per-request MPP machine payments -- fiat SPTs or USDC -- for account-free agent callers). API keys (api_sk_, hashed, shown once) ride the same auth context as sessions.
