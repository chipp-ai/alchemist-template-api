...
**Dev login (terminal/agent testing):** there is no SMTP / inbox harness in dev, so the email-OTP code never reaches an inbox. Use the dev-login escape hatch:
...
**Stack:**
- **API:** Deno 2 + Hono 4 (headless, no frontend)
...
- **ALWAYS use `./scripts/dev.sh --api-port __API_PORT__`** -- port is required (no defaults), logs go to `.scratch/logs/`.
...
import {
  ...
  getSignedDownloadUrl,     // client-facing read URL (default 1h, max 7d)
  getSignedUploadUrl,       // client direct PUT URL (default 15m, max 7d)
  isStorageConfigured,
...
This is a headless API template (no web/ SPA). Skip browser verification steps.

## Agent verification toolkit — pick the right tool

When verifying a change against the running app, use `dev_app_state` (via MCP or curl to `/api/dev/app-state`) and `bash` + `curl`. This is a headless API template — there is no SPA or browser UI to drive.

### Primary tool — `dev_app_state`
...
GETs `/api/dev/app-state` on the running customer app. Returns:

- **Server side** — the last 20 HTTP requests with method/path/status/duration, and any captured server errors.

**Use this first** for any "is the running app in the state I expect?" question. It answers "did my last PATCH succeed / did the server throw" with a single tool call.
...
```bash
curl -s http://localhost:__API_PORT__/health
curl -s -b /tmp/jar.txt http://localhost:__API_PORT__/api/auth/me
```

Verify response bodies, not just status codes. Use `jq` to inspect `.server.recentRequests` from `dev_app_state` to confirm calls landed.
